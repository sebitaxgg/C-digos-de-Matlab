# -*- coding: utf-8 -*-
"""
Created on Sat Apr 12 15:40:38 2025
...
# Energías máximas a considerar
energias_max = [-2.3]  # Puedes poner uno solo o más
...
for idx, energia_max in enumerate(energias_max):
    ...
    for numero in orden_deseado:
        ...
        magnitud_obs = datos_obs[:, 5]  # Columna observada
    ...
    ax_sigma.set_title(f"Traza Hopping σ \n(energia_max = {energia_max})")
    ...
    ax_max.set_title(f"Máximo Traza \n(energia_max = {energia_max})")
    ax_max.set_xlabel('N')
...
plt.scatter(energia_filtradas,-residuos,color='dodgerblue', s=1)
data = np.column_stack((energia_filtradas,-residuos))
np.savetxt("Grafica_160.txt", data, fmt='%.6f', delimiter='\t', header='x\ty')